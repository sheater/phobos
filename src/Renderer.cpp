...
Texture *Renderer::createTexture(unsigned int width, unsigned int height, void *pixels)
{
  Texture *tex = new Texture(width, height, pixels);
...
void Renderer::useBaseProgram(const glm::mat4x4 &transform, Material* material)
{
  m_baseProgram->use();

  m_baseProgram->setUniformMatrix("projectionMat", m_projectionMatrix);
  m_baseProgram->setUniformMatrix("viewMat", m_viewMatrix);
  m_baseProgram->setUniformMatrix("modelMat", transform);

  Light light;

  light.position = glm::vec3(4.0f, 4.5f, 2.5f);
  ...
  glm::vec4 diffuseColor = material ? material->diffuseColor : glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
